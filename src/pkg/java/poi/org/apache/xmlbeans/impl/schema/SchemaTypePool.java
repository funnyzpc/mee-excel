...
class SchemaTypePool {
    ...
    void readHandlePool(XsbReader reader) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (code) {
                case SchemaTypeSystemImpl.FILETYPE_SCHEMATYPE:
                    result = new SchemaType.Ref(typeSystem, handle);
                    break;
                case SchemaTypeSystemImpl.FILETYPE_SCHEMAELEMENT:
                    result = new SchemaGlobalElement.Ref(typeSystem, handle);
                    break;
                case SchemaTypeSystemImpl.FILETYPE_SCHEMAATTRIBUTE:
                    result = new SchemaGlobalAttribute.Ref(typeSystem, handle);
                    break;
                case SchemaTypeSystemImpl.FILETYPE_SCHEMAMODELGROUP:
                    result = new SchemaModelGroup.Ref(typeSystem, handle);
                    break;
                case SchemaTypeSystemImpl.FILETYPE_SCHEMAATTRIBUTEGROUP:
                    result = new SchemaAttributeGroup.Ref(typeSystem, handle);
                    break;
                case SchemaTypeSystemImpl.FILETYPE_SCHEMAIDENTITYCONSTRAINT:
