...
import poi.org.apache.xmlbeans.SchemaLocalAttribute;
import poi.org.apache.xmlbeans.SchemaLocalElement;
import poi.org.apache.xmlbeans.SchemaModelGroup;
import poi.org.apache.xmlbeans.SchemaParticle;
import poi.org.apache.xmlbeans.SchemaStringEnumEntry;
import poi.org.apache.xmlbeans.SchemaType;
import poi.org.apache.xmlbeans.impl.common.QNameHelper;
import poi.org.apache.xmlbeans.impl.util.LongUTFDataInputStream;
import poi.org.apache.xmlbeans.impl.util.LongUTFDataOutputStream;
import poi.org.apache.xmlbeans.impl.values.XmlObjectBase;
import poi.org.apache.xmlbeans.impl.xb.xsdschema.AttributeGroupDocument;
import poi.org.apache.xmlbeans.impl.xb.xsdschema.GroupDocument;
import poi.org.apache.xmlbeans.soap.SOAPArrayType;
import poi.org.apache.xmlbeans.soap.SchemaWSDLArrayType;
...
import poi.org.apache.xmlbeans.SchemaProperty;
import poi.org.apache.xmlbeans.SchemaTypeLoaderException;
...
class XsbReader {
    ...
    public XsbReader(SchemaTypeSystemImpl typeSystem, String handle, int filetype) {
        ...
        int magic = readInt();
        if (magic != SchemaTypeSystemImpl.DATA_BABE) {
        ...
        if (_majorver != SchemaTypeSystemImpl.MAJOR_VERSION) {
            throw new SchemaTypeLoaderException("XML-BEANS compiled schema: Wrong major version - expecting " + SchemaTypeSystemImpl.MAJOR_VERSION + ", got " + _majorver, typeSystem.getName(), handle, SchemaTypeLoaderException.WRONG_MAJOR_VERSION);
        }

        if (_minorver > SchemaTypeSystemImpl.MINOR_VERSION) {
            throw new SchemaTypeLoaderException("XML-BEANS compiled schema: Incompatible minor version - expecting up to " + SchemaTypeSystemImpl.MINOR_VERSION + ", got " + _minorver, typeSystem.getName(), handle, SchemaTypeLoaderException.WRONG_MINOR_VERSION);
        }
    ...
    public SchemaGlobalElement finishLoadingElement() {
        try {
            ...
            impl.setTransitionRules(transitionRules,
                (particleFlags & SchemaTypeSystemImpl.FLAG_PART_SKIPPABLE) != 0);
            impl.setNameAndTypeRef(name, readTypeRef());
            impl.setDefault(readString(), (particleFlags & SchemaTypeSystemImpl.FLAG_PART_FIXED) != 0, null);
            if (atLeast(2, 16, 0)) {
                impl.setDefaultValue(readXmlValueObject());
            }
            impl.setNillable((particleFlags & SchemaTypeSystemImpl.FLAG_PART_NILLABLE) != 0);
            impl.setBlock((particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKEXT) != 0,
                (particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKREST) != 0,
                (particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKSUBST) != 0);
            impl.setWsdlArrayType(readSOAPArrayType());
            impl.setAbstract((particleFlags & SchemaTypeSystemImpl.FLAG_PART_ABSTRACT) != 0);
            impl.setAnnotation(readAnnotation(container));
            impl.setFinal(
                (particleFlags & SchemaTypeSystemImpl.FLAG_PART_FINALEXT) != 0,
                (particleFlags & SchemaTypeSystemImpl.FLAG_PART_FINALREST) != 0);
    ...
    public SchemaType finishLoadingType() {
        try {
            ...
            switch (readShort()) {
                case SchemaTypeSystemImpl.FIELD_GLOBAL:
                    impl.setContainerFieldRef(readHandle());
                    break;
                case SchemaTypeSystemImpl.FIELD_LOCALATTR:
                    impl.setContainerFieldIndex((short) 1, readShort());
                    break;
                case SchemaTypeSystemImpl.FIELD_LOCALELT:
            ...
            boolean isComplexType = ((flags & SchemaTypeSystemImpl.FLAG_SIMPLE_TYPE) == 0);
            impl.setCompiled((flags & SchemaTypeSystemImpl.FLAG_COMPILED) != 0);
            impl.setDocumentType((flags & SchemaTypeSystemImpl.FLAG_DOCUMENT_TYPE) != 0);
            impl.setAttributeType((flags & SchemaTypeSystemImpl.FLAG_ATTRIBUTE_TYPE) != 0);
            impl.setSimpleType(!isComplexType);

            int complexVariety = SchemaType.NOT_COMPLEX_TYPE;
            if (isComplexType) {
                impl.setAbstractFinal((flags & SchemaTypeSystemImpl.FLAG_ABSTRACT) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_EXT) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_REST) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_LIST) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_UNION) != 0);
                impl.setBlock((flags & SchemaTypeSystemImpl.FLAG_BLOCK_EXT) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_BLOCK_REST) != 0);

                impl.setOrderSensitive((flags & SchemaTypeSystemImpl.FLAG_ORDER_SENSITIVE) != 0);
            ...
            if (!isComplexType || complexVariety == SchemaType.SIMPLE_CONTENT) {
                ...
                boolean isStringEnum = ((flags & SchemaTypeSystemImpl.FLAG_STRINGENUM) != 0);

                impl.setOrdered((flags & SchemaTypeSystemImpl.FLAG_ORDERED) != 0 ? SchemaType.UNORDERED : ((flags & SchemaTypeSystemImpl.FLAG_TOTAL_ORDER) != 0 ? SchemaType.TOTAL_ORDER : SchemaType.PARTIAL_ORDER));
                impl.setBounded((flags & SchemaTypeSystemImpl.FLAG_BOUNDED) != 0);
                impl.setFinite((flags & SchemaTypeSystemImpl.FLAG_FINITE) != 0);
                impl.setNumeric((flags & SchemaTypeSystemImpl.FLAG_NUMERIC) != 0);
                impl.setUnionOfLists((flags & SchemaTypeSystemImpl.FLAG_UNION_OF_LISTS) != 0);
                impl.setSimpleFinal((flags & SchemaTypeSystemImpl.FLAG_FINAL_REST) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_LIST) != 0,
                    (flags & SchemaTypeSystemImpl.FLAG_FINAL_UNION) != 0);
                ...
                impl.setPatternFacet((flags & SchemaTypeSystemImpl.FLAG_HAS_PATTERN) != 0);

                int patternCount = readShort();
                RegularExpression[] patterns = new RegularExpression[patternCount];
                for (int i = 0; i < patternCount; i++) {
                    patterns[i] = new RegularExpression(readString(), "X");
                }
    ...
    void writeTypeData(SchemaType type) {
        ...
        if (type.isSimpleType()) {
            flags |= SchemaTypeSystemImpl.FLAG_SIMPLE_TYPE;
        }
        if (type.isDocumentType()) {
            flags |= SchemaTypeSystemImpl.FLAG_DOCUMENT_TYPE;
        }
        if (type.isAttributeType()) {
            flags |= SchemaTypeSystemImpl.FLAG_ATTRIBUTE_TYPE;
        }
        if (type.ordered() != SchemaType.UNORDERED) {
            flags |= SchemaTypeSystemImpl.FLAG_ORDERED;
        }
        if (type.ordered() == SchemaType.TOTAL_ORDER) {
            flags |= SchemaTypeSystemImpl.FLAG_TOTAL_ORDER;
        }
        if (type.isBounded()) {
            flags |= SchemaTypeSystemImpl.FLAG_BOUNDED;
        }
        if (type.isFinite()) {
            flags |= SchemaTypeSystemImpl.FLAG_FINITE;
        }
        if (type.isNumeric()) {
            flags |= SchemaTypeSystemImpl.FLAG_NUMERIC;
        }
        if (type.hasStringEnumValues()) {
            flags |= SchemaTypeSystemImpl.FLAG_STRINGENUM;
        }
        if (((SchemaTypeImpl) type).isUnionOfLists()) {
            flags |= SchemaTypeSystemImpl.FLAG_UNION_OF_LISTS;
        }
        if (type.hasPatternFacet()) {
            flags |= SchemaTypeSystemImpl.FLAG_HAS_PATTERN;
        }
        if (type.isOrderSensitive()) {
            flags |= SchemaTypeSystemImpl.FLAG_ORDER_SENSITIVE;
        }

        if (type.blockExtension()) {
            flags |= SchemaTypeSystemImpl.FLAG_BLOCK_EXT;
        }
        if (type.blockRestriction()) {
            flags |= SchemaTypeSystemImpl.FLAG_BLOCK_REST;
        }
        if (type.finalExtension()) {
            flags |= SchemaTypeSystemImpl.FLAG_FINAL_EXT;
        }
        if (type.finalRestriction()) {
            flags |= SchemaTypeSystemImpl.FLAG_FINAL_EXT;
        }
        if (type.finalList()) {
            flags |= SchemaTypeSystemImpl.FLAG_FINAL_LIST;
        }
        if (type.finalUnion()) {
            flags |= SchemaTypeSystemImpl.FLAG_FINAL_UNION;
        }
        if (type.isAbstract()) {
            flags |= SchemaTypeSystemImpl.FLAG_ABSTRACT;
        }
    ...
    void loadParticle(SchemaParticleImpl result, int particleType) {
        ...
        switch (particleType) {
            ...
            case SchemaParticle.ELEMENT:
                ...
                lresult.setDefault(readString(), (particleFlags & SchemaTypeSystemImpl.FLAG_PART_FIXED) != 0, null);
                if (atLeast(2, 16, 0)) {
                    lresult.setDefaultValue(readXmlValueObject());
                }
                lresult.setNillable((particleFlags & SchemaTypeSystemImpl.FLAG_PART_NILLABLE) != 0);
                lresult.setBlock((particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKEXT) != 0,
                    (particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKREST) != 0,
                    (particleFlags & SchemaTypeSystemImpl.FLAG_PART_BLOCKSUBST) != 0);
                lresult.setWsdlArrayType(readSOAPArrayType());
                lresult.setAbstract((particleFlags & SchemaTypeSystemImpl.FLAG_PART_ABSTRACT) != 0);
    ...
    void writeParticleData(SchemaParticle part) {
        ...
        short flags = 0;
        if (part.isSkippable()) {
            flags |= SchemaTypeSystemImpl.FLAG_PART_SKIPPABLE;
        }
        if (part.getParticleType() == SchemaParticle.ELEMENT) {
            SchemaLocalElement lpart = (SchemaLocalElement) part;
            if (lpart.isFixed()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_FIXED;
            }
            if (lpart.isNillable()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_NILLABLE;
            }
            if (lpart.blockExtension()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_BLOCKEXT;
            }
            if (lpart.blockRestriction()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_BLOCKREST;
            }
            if (lpart.blockSubstitution()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_BLOCKSUBST;
            }
            if (lpart.isAbstract()) {
                flags |= SchemaTypeSystemImpl.FLAG_PART_ABSTRACT;
            }

            if (lpart instanceof SchemaGlobalElement) {
                SchemaGlobalElement gpart = (SchemaGlobalElement) lpart;
                if (gpart.finalExtension()) {
                    flags |= SchemaTypeSystemImpl.FLAG_PART_FINALEXT;
                }
                if (gpart.finalRestriction()) {
                    flags |= SchemaTypeSystemImpl.FLAG_PART_FINALREST;
                }
            }
        }
        writeShort(flags);
    ...
    SchemaProperty readPropertyData() {
        ...
        prop.setAttribute((propflags & SchemaTypeSystemImpl.FLAG_PROP_ISATTR) != 0);
        ...
        prop.setExtendsJava(readTypeRef(),
            (propflags & SchemaTypeSystemImpl.FLAG_PROP_JAVASINGLETON) != 0,
            (propflags & SchemaTypeSystemImpl.FLAG_PROP_JAVAOPTIONAL) != 0,
            (propflags & SchemaTypeSystemImpl.FLAG_PROP_JAVAARRAY) != 0);
        if (atMost(2, 19, 0)) {
    ...
    void writePropertyData(SchemaProperty prop) {
        ...
        writeShort((prop.isAttribute() ? SchemaTypeSystemImpl.FLAG_PROP_ISATTR : 0) |
                   (prop.extendsJavaSingleton() ? SchemaTypeSystemImpl.FLAG_PROP_JAVASINGLETON : 0) |
                   (prop.extendsJavaOption() ? SchemaTypeSystemImpl.FLAG_PROP_JAVAOPTIONAL : 0) |
                   (prop.extendsJavaArray() ? SchemaTypeSystemImpl.FLAG_PROP_JAVAARRAY : 0));
