...
import poi.org.apache.xmlbeans.impl.common.NameUtil;
import poi.org.apache.xmlbeans.impl.repackage.Repackager;
import poi.org.apache.xmlbeans.InterfaceExtension;
import poi.org.apache.xmlbeans.PrePostExtension;
import poi.org.apache.xmlbeans.SchemaCodePrinter;
...
public final class SchemaTypeCodePrinter implements SchemaCodePrinter {
    ...
    void emit(String s, XmlOptions.BeanMethod method) throws IOException {
        Set<XmlOptions.BeanMethod> partMet = opt == null ? null : opt.getCompilePartialMethod();
    ...
    void printPropertySetters(SchemaProperty prop) throws IOException {
        ...
        if (singleton) {
            ...
            emit("void set" + propertyName + "(" + type + " " + safeVarName + ");", XmlOptions.BeanMethod.SET);

            if (!xmltype) {
                printJavaDoc((several ? "Sets (as xml) first " : "Sets (as xml) the ") + propdesc, XmlOptions.BeanMethod.XSET);
                emit("void xset" + propertyName + "(" + xtype + " " + safeVarName + ");", XmlOptions.BeanMethod.XSET);
            }

            if (xmltype && !several) {
                printJavaDoc("Appends and returns a new empty " + propdesc, XmlOptions.BeanMethod.ADD_NEW);
                emit(xtype + " addNew" + propertyName + "();", XmlOptions.BeanMethod.ADD_NEW);
            }

            if (nillable) {
                printJavaDoc((several ? "Nils the first " : "Nils the ") + propdesc, XmlOptions.BeanMethod.SET_NIL);
                emit("void setNil" + propertyName + "();", XmlOptions.BeanMethod.SET_NIL);
            }
        }
        ...
        if (optional) {
            ...
            emit("void unset" + propertyName + "();", XmlOptions.BeanMethod.UNSET);
        }
        ...
        if (several) {
            ...
            emit("void set" + arrayName + "(int i, " + type + " " + safeVarName + ");", XmlOptions.BeanMethod.SET_IDX);
            ...
            if (!xmltype) {
                ...
                emit("void xset" + arrayName + "(int i, " + xtype + " " + safeVarName + ");", XmlOptions.BeanMethod.XSET_IDX);
            }
            ...
            if (!xmltype) {
                ...
                emit("void insert" + propertyName + "(int i, " + type + " " + safeVarName + ");", XmlOptions.BeanMethod.INSERT_IDX);

                printJavaDoc("Appends the value as the last " + propdesc, XmlOptions.BeanMethod.ADD);
                emit("void add" + propertyName + "(" + type + " " + safeVarName + ");", XmlOptions.BeanMethod.ADD);
            }
    ...
    void printListGetterImpl(String propdesc, String propertyName, String wrappedType, boolean xmltype, boolean xget)
            throws IOException {
        Set<XmlOptions.BeanMethod> bmList = (opt == null) ? null : opt.getCompilePartialMethod();
        if (bmList != null && !bmList.contains(xget ? XmlOptions.BeanMethod.XGET_LIST : XmlOptions.BeanMethod.GET_LIST)) {
            return;
        }
        ...
        indent();
        if (bmList == null || bmList.contains(xget ? XmlOptions.BeanMethod.XGET_IDX : XmlOptions.BeanMethod.GET_IDX)) {
            emit("this::" + (xget ? "xget" : "get") + arrayName + ",");
        } else {
            emit("null,");
        }
        if (bmList == null || bmList.contains(xget ? XmlOptions.BeanMethod.XSET_IDX : XmlOptions.BeanMethod.SET_IDX)) {
            emit("this::" + (xget ? "xset" : "set") + arrayName + ",");
        } else {
            emit("null,");
        }
        if (bmList == null || bmList.contains((xmltype || xget) ? XmlOptions.BeanMethod.INSERT_NEW_IDX : XmlOptions.BeanMethod.INSERT_IDX)) {
            emit("this::insert" + ((xmltype || xget) ? "New" : "") + propertyName + ",");
        } else {
            emit("null,");
        }
        if (bmList == null || bmList.contains(XmlOptions.BeanMethod.REMOVE_IDX)) {
    ...
    void printGetterImpls(SchemaProperty prop, Map<SchemaProperty, Identifier> propMap)
        ...
        if (prop.extendsJavaSingleton()) {
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.GET)) {
                // Value getProp()
                if(opt.isCompileAnnotationAsJavadoc() && propertyDocumentation != null && propertyDocumentation.length() > 0){
                    printJavaDocParagraph(propertyDocumentation);
                } else {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.XGET))) {
                // Value xgetProp()
            ...
            if (nillable && (bmList == null || bmList.contains(XmlOptions.BeanMethod.IS_NIL))) {
                // boolean isNilProp()
        ...
        if (several) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.XGET_IDX))) {
    ...
    void printSetterImpls(SchemaProperty prop, Map<SchemaProperty, Identifier> propMap, SchemaType sType)
        ...
        if (prop.extendsJavaSingleton()) {
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.SET)) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.XSET))) {
            ...
            if (xmltype && !several && (bmList == null || bmList.contains(XmlOptions.BeanMethod.ADD_NEW))) {
                // Value addNewProp()
            ...
            if (nillable && (bmList == null || bmList.contains(XmlOptions.BeanMethod.SET_NIL))) {
        ...
        if (prop.extendsJavaOption() && (bmList == null || bmList.contains(XmlOptions.BeanMethod.UNSET))) {
        ...
        if (several) {
            ...
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.SET_ARRAY)) {
                if (xmltype) {
            ...
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.SET_IDX)) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.XSET_ARRAY))) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.XSET_IDX))) {
            ...
            if (nillable && (bmList == null || bmList.contains(XmlOptions.BeanMethod.SET_NIL_IDX))) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.INSERT_IDX))) {
            ...
            if (!xmltype && (bmList == null || bmList.contains(XmlOptions.BeanMethod.ADD))) {
            ...
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.INSERT_NEW_IDX)) {
            ...
            if (bmList == null || bmList.contains(XmlOptions.BeanMethod.ADD_NEW)) {
