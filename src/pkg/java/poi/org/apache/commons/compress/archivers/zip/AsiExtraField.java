...
public class AsiExtraField implements ZipExtraField, UnixStat, Cloneable {
    ...
    private static final int MIN_SIZE = ZipConstants.WORD + ZipConstants.SHORT + ZipConstants.WORD + ZipConstants.SHORT + ZipConstants.SHORT;
    ...
    public byte[] getLocalFileDataData() {
        ...
        final byte[] data = new byte[getLocalFileDataLength().getValue() - ZipConstants.WORD];
        System.arraycopy(ZipShort.getBytes(getMode()), 0, data, 0, 2);

        final byte[] linkArray = getLinkedFile().getBytes(Charset.defaultCharset()); // Uses default charset - see class Javadoc
        // CheckStyle:MagicNumber OFF
        System.arraycopy(ZipLong.getBytes(linkArray.length), 0, data, 2, ZipConstants.WORD);
        ...
        final byte[] result = new byte[data.length + ZipConstants.WORD];
        System.arraycopy(ZipLong.getBytes(checksum), 0, result, 0, ZipConstants.WORD);
        System.arraycopy(data, 0, result, ZipConstants.WORD, data.length);
        return result;
    }
    ...
    public ZipShort getLocalFileDataLength() {
        // @formatter:off
        return new ZipShort(ZipConstants.WORD      // CRC
                          + 2         // Mode
                          + ZipConstants.WORD      // SizDev
                          + 2         // UID
                          + 2         // GID
    ...
    public void parseFromLocalFileData(final byte[] data, final int offset, final int length) throws ZipException {
        ...
        final byte[] tmp = new byte[length - ZipConstants.WORD];
        System.arraycopy(data, offset + ZipConstants.WORD, tmp, 0, length - ZipConstants.WORD);
        crc.reset();
        crc.update(tmp);
