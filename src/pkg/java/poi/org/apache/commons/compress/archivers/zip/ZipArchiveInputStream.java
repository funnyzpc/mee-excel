 ...
 * under the License.
 */
package poi.org.apache.commons.compress.archivers.zip;

import poi.org.apache.commons.compress.archivers.ArchiveEntry;
import poi.org.apache.commons.compress.archivers.ArchiveInputStream;
import poi.org.apache.commons.compress.compressors.bzip2.BZip2CompressorInputStream;
import poi.org.apache.commons.compress.compressors.deflate64.Deflate64CompressorInputStream;
import poi.org.apache.commons.compress.utils.ArchiveUtils;
import poi.org.apache.commons.compress.utils.IOUtils;
...
public class ZipArchiveInputStream extends ArchiveInputStream<ZipArchiveEntry> implements InputStreamStatistics {
     ...
     */
    private static final long TWO_EXP_32 = ZipConstants.ZIP64_MAGIC + 1;
    ...
    private final byte[] twoDwordBuf = new byte[2 * ZipConstants.DWORD];
    ...
    public ZipArchiveEntry getNextZipEntry() throws IOException {
        ...
        off += ZipConstants.SHORT; // NOSONAR - assignment as documentation
    ...
    private boolean isApkSigningBlock(final byte[] suspectLocalFileHeader) throws IOException {
        ...
        BigInteger toSkip = len.add(BigInteger.valueOf(ZipConstants.DWORD - suspectLocalFileHeader.length - (long) APK_SIGNING_BLOCK_MAGIC.length));
    ...
    private void readDataDescriptor() throws IOException {
        ...
        final ZipLong potentialSig = new ZipLong(twoDwordBuf, ZipConstants.DWORD);
        if (potentialSig.equals(ZipLong.CFH_SIG) || potentialSig.equals(ZipLong.LFH_SIG)) {
            ...
            size = ZipLong.getValue(twoDwordBuf, ZipConstants.WORD);
            if (size < 0) {
                throw new ZipException("broken archive, entry with negative size");
            }
            ...
            current.entry.setCompressedSize(size);
            size = ZipEightByteInteger.getLongValue(twoDwordBuf, ZipConstants.DWORD);
            if (size < 0) {
                throw new ZipException("broken archive, entry with negative size");
            }
            current.entry.setSize(size);
    ...
    private void readStoredEntry() throws IOException {
        ...
        final int ddLen = current.usesZip64 ? ZipConstants.WORD + 2 * ZipConstants.DWORD : 3 * ZipConstants.WORD;
    ...
    private void skipRemainderOfArchive(final boolean read) throws IOException {
        ...
        if (foundEocd) {
            realSkip((long) ZipFile.MIN_EOCD_SIZE - ZipConstants.WORD /* signature */ - ZipConstants.SHORT /* comment len */);
            readFully(shortBuf);
            // file comment
